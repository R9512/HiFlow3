...
ggsave("EXP1_GAS_USAGE.png", plot = p, width = 10, height = 10, dpi = 300)
p
...
facet_labels <- c(
  "HiFlow3" = "HiFlow3 (MCI = 2)",
  "Flower" = "Flower"
  # add more entries here if you have additional implementations
)
...
p <- ggplot(df, aes(x = FLIP.PROP, y = ACCURACY,
  ...
  geom_point(size = 3) +
  geom_line(size = 1) +
  labs(title = "Data Poisoning - Test Accuracy vs Flip, Proportion ",
       x = "Flip, Proportion",
       y = "Test Accuracy (%)",
       color = "Dataset",
       shape = "Model")+ #+scale_linetype_manual(values = c("Flower" = "dotted", "HiFlow3" = "solid")) 
      facet_wrap(~ IMPLIMENTATION,labeller = as_labeller(facet_labels))+
  theme(
    panel.grid.major = element_blank(), 
    panel.grid.minor = element_blank(), 
    panel.background = element_blank(), 
...
df <- df %>%
  mutate(FLIP.PROP = sapply(FLIP.PROP, function(x) {
    split_values <- strsplit(x, ",")[[1]]  # Split by comma
    first_element_modified <- as.numeric(split_values[1]) * 3  # Multiply first element by 3CAREFULL SJ
...
p <- ggplot(df, aes(x = FLIP.PROP, y = ACCURACY,
  ...
  geom_point(size = 3) +
  geom_line(size = 1) +
  labs(title = "Model Poisoning - Test Accuracy vs Flip, Proportion ",
       x = "Flip, Proportion",
       y = "Test Accuracy (%)",
       color = "Dataset",
       shape = "Model")+ #+scale_linetype_manual(values = c("Flower" = "dotted", "HiFlow3" = "solid")) 
