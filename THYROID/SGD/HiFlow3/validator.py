...
def prepare_data(data_part):
	...
	print("VALIDATPR:",data_part)
	if(not os.path.exists("./data/"+data_part+".csv")):#CHecking for malicious
		data_part+="_m"
...
def model_tester(round_num):
	...
	if TEST_MODEL == 0:	
		TEST_MODEL = SGDClassifier(penalty="l2",max_iter=1,warm_start=True,class_weight={1:0.542,0:6.464})
...
#Need to add a check here to prove that the specific guy is not malicious
