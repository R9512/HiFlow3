...
def get_evaluate_fn(model):
    ...
    _, X_test,_, y_test = load_data("./thyroid_data/test.csv","lable",2)
...
if __name__ == "__main__":
    ...
    model = SGDClassifier(penalty="l2",max_iter=2,)
    set_initial_params(model,2,14)
    strategy = fl.server.strategy.FedAvg(min_available_clients=50,evaluate_fn=get_evaluate_fn(model),on_fit_config_fn=fit_round,)
