...
def mopper(flip,prop):
    ...
    """
    base = "./Flower/"
    source = base+"EXP2_DATA_POISON/"+str(flip)+","+str(prop)
    os.mkdir(source)
    ...
    for x in os.listdir(base):
        if(".pickle" in x):
            move(base+x,source)
    print(source)
    tester("HiFlow3","HiFlow3/EXP2_DATA_POISON/"+str(flip)+","+str(prop))
    tester("Flower","Flower/EXP2_DATA_POISON/"+str(flip)+","+str(prop))
    #Remove TXTs from here


def auto_data_poisoning():
    """
        - Generic Functions that make things work

    """
    global TEST_DATA,GROUND_TRUTH
    os.mkdir("./HiFlow3/EXP2_DATA_POISON")
    os.mkdir("./Flower/EXP2_DATA_POISON")

    #just setting situations for Exp2
    for flip in range(2,3):
        for prop in range(2,5,2):#20,
            config = toml.load("./HiFlow3/config.toml")
            config["EXPERIMENT"]["FLIP_NUM"] = flip
...
guardrail()
config = toml.load("./HiFlow3/config.toml")
config["EXPERIMENT"]["A_TYPE"] = "DATA"
config["EXPERIMENT"]["FLIP_NUM"] = 0
config["EXPERIMENT"]["PROPORTION"] = 0
config["EXPERIMENT"]["MALICIOUS"] = [ 0 for x in range(15)]
config["COORDINATOR"]["ZETA"] = 2
with open("./HiFlow3/config.toml", 'w') as f:
    toml.dump(config, f)
rmtree("./HiFlow3/EXP2_DATA_POISON",ignore_errors=True)
rmtree("./Flower/EXP2_DATA_POISON",ignore_errors=True)
